...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  default_node_pool {
    ...
    max_count = 2
    min_count = 1
    orchestrator_version = data.azurerm_kubernetes_service_versions.current
    os_disk_size_gb = 30
    type = "VirtualMachineScaleSet" // AvailabilitySet and VirtualMachineScaleSets. Defaults to VirtualMachineScaleSets
 ...
 azure_active_directory_role_based_access_control {
   ...
   # identity block can either be systemAssigned or Service Principal
  identity {
    type = "SystemAssigned"
  }
